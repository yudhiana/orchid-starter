...
type HasSellerRolesInput struct {
	Injection *bootstrap.DirectInjection
	Ctx       context.Context
}

func (d *Directive) HasSellerRoles(ctx context.Context, obj any, next graphql.Resolver, roles []model.SellerRoles) (res any, err error) {
...
func (i *HasSellerRolesInput) CheckCacheRoles(roles []model.SellerRoles) bool {
	var (
	...
	if errGet != nil || sellerRolesString == "" {
		i.Injection.Log.Warn("role seller on redis is empty")
		return false
	}

	if _, ok := sellerRolesString.(string); !ok {
		i.Injection.Log.Warn("role seller on redis is not string")
		return false
	}
