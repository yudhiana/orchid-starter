...
type DirectInjection struct {
	...
	Tracer    *openTelemetry.OTel
}

// NewApplicationContainer creates a new dependency injection container with proper error handling
func NewApplicationContainer(cfg *config.LocalConfig) (*DirectInjection, error) {
	...
	if common.GetBoolEnv("OTEL_ACTIVE", false) {
		...
		di.Tracer = openTelemetry.InitOTelTracer(context.Background())
	}
...
func (di *DirectInjection) GetTracer() *openTelemetry.OTel {
	return di.Tracer
}
